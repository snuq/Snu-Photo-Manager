...
def get_file_info(file_info, import_mode=False):
...
class SettingMultiDirectory(SettingItem):
    ...
    def add_directory(self, *_):
        self.modified = True
        if self.value:
            self.value = self.value+';'+agnostic_path(self.filepopup.content.filename)
        else:
            self.value = agnostic_path(self.filepopup.content.filename)
...
class PhotoManager(App):
    ...
    def refresh_photo(self, fullpath, force=False, no_photoinfo=False):
        ...
        if old_photoinfo:
            ...
            if os.path.isfile(photo_filename):
                #file still exists
                new_photoinfo = get_file_info([old_photoinfo[0], old_photoinfo[2]], import_mode=True)
                if new_photoinfo[7] != old_photoinfo[7] or force:
    ...
    def database_clean(self, deep=False):
        ...
        for folder in folders:
            folder_renamed = local_path(folder[1])
            for database in databases:
                if os.path.isdir(database) or deep:
        ...
        for index, file_info in enumerate(files):
            ...
            if extension in imagetypes or extension in movietypes:
                ...
                if not exists:
                    ...
                    refreshed = self.refresh_photo(file_info[0], no_photoinfo=True)
                    if refreshed:
